...
export function sendErrorResponse(error: Error, res: Response) {
    ...
    if (error.name === _VALIDATION_ERROR_NAME_ || error.name === _DOMAIN_VALIDATION_ERROR_NAME_) {
        status = 400;
        message = `Bad Request - ${error.message}`;
    } else if (error.name === _FORBIDDEN_ERROR_NAME_) {
        status = 403;
        message = `Forbidden - ${error.message}`;
    } else if (error.name === _NOT_FOUND_ERROR_NAME_ || error.name === _DOMAIN_NOT_FOUND_ERROR_NAME_ || error.name === _DATABASE_NOT_FOUND_ERROR_NAME_) {
        status = 404;
